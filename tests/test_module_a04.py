...
def rule_ids(a04, php: str) -> list[str]:
...
def test_md5_in_password_context_is_credential_hashing(a04) -> None:
    assert rule_ids(a04, "<?php $hash = md5($password);") == ["A04-HASH-001"]


def test_md5_without_password_context_is_general_weak_hash(a04) -> None:
    assert rule_ids(a04, "<?php $checksum = md5($file_contents);") == ["A04-HASH-002"]


def test_sha1_comparison_near_password_identifier(a04) -> None:
    assert rule_ids(a04, "<?php if (sha1($input) === $stored_password_hash) { $ok = true; }") == ["A04-HASH-001"]


def test_crypt_without_salt_is_flagged(a04) -> None:
    assert rule_ids(a04, "<?php $h = crypt($pw);") == ["A04-HASH-001"]


def test_crypt_with_weak_literal_salt_is_flagged(a04) -> None:
    assert rule_ids(a04, "<?php $h = crypt($pw, '$1$abc$');") == ["A04-HASH-001"]


def test_crypt_with_strong_salt_is_clean(a04) -> None:
    assert rule_ids(a04, "<?php $h = crypt($pw, '$2y$10$abcdefghijklmnopqrstuv');") == []


def test_crypt_with_dynamic_salt_is_not_guessed(a04) -> None:
    assert rule_ids(a04, "<?php $h = crypt($pw, $salt);") == []


def test_password_hash_is_clean(a04) -> None:
    assert rule_ids(a04, "<?php $h = password_hash($password, PASSWORD_DEFAULT);") == []

# endregion
# region randmoness

def test_insecure_random_functions_flagged(a04) -> None:
    assert rule_ids(a04, "<?php $t = uniqid(rand(), true);") == ["A04-RAND-001"] * 2


def test_random_bytes_and_int_is_clean(a04) -> None:
    assert rule_ids(a04, "<?php $t = bin2hex(random_bytes(32)); $n = random_int(0, 9);") == []
...
def test_mcrypt_is_deprecated(a04) -> None:
    ...
    assert rule_ids(a04, php) == ["A04-CRYPTO-001"]


def test_openssl_with_weak_cipher_flagged(a04) -> None:
    assert rule_ids(a04, "<?php $c = openssl_encrypt($data, 'DES-ECB', $key);") == ["A04-CRYPTO-002"]
    assert rule_ids(a04, "<?php $c = openssl_encrypt($data, 'aes-128-ecb', $key);") == ["A04-CRYPTO-002"]


def test_openssl_with_aead_cipher_clean(a04) -> None:
    assert rule_ids(a04, "<?php $c = openssl_encrypt($data, 'aes-256-gcm', $key, 0, $iv, $tag);") == []
...
def test_secret_in_array_key_and_define_flagged(a04) -> None:
    ...
    assert rule_ids(a04, php) == ["A04-SECRET-001"] * 2


def test_non_literal_or_short_values_clean(a04) -> None:
    assert rule_ids(a04, "<?php $password = $_POST['password'];") == []
    assert rule_ids(a04, "<?php $token = \"prefix_$id\";") == []
    assert rule_ids(a04, "<?php $password = '';") == []


def test_unrelated_assignment_clean(a04) -> None:
    assert rule_ids(a04, "<?php $greeting = 'hello world';") == []
